...
const supreme = require('supremenewyork-api');
        ...
        if (!err) {
            ...
            $('img').each(function(i, element) {
                ...
                request(link, function(err, resp, html, rrr, body) {
                    ...
                    var metadata = {
                        title:$('.product-name > a.name-link').eq(1).html(),
                        // style: $('.name-link').parent(".product-style").text().split(/(?=[A-Z])/)[3],
                        // style:$('.product-name > a').html(),
                        style:$('.product-style > a.name-link').text(),
...
router.get('/ax', (req,res)=>{
  ...
  const target= 'https://www.supremenewyork.com/shop/all/pants'

   const prods = api.getItems("sweatshirts", (prod) => res.json(prod))
// // const getdata =  () => {
//     axios.get(target)
//     .then(result =>{
...
router.get('/nono', (req,res)=>{


      
  const opener = (category, url  ) => {    
                var splits  =  url.split(`${category}`)
                var res = ('https://www.supremenewyork.com/shop/').concat(`${category}`).concat(splits[1])
               open(res)
           }
...
const search = (color)=>{
     ...
     let promise = new Promise(function(resolve, reject) {
         ...
         const info = (products) => {                        
                    let arr = []
                     products.map( p => {
                        ...
                        // set condition for finding item , style cand color adont work 
                            if (p.title ==  color){ 
                            arr.push(p)
        ...
        const getProducts =  api.getItems("sweatshirts", info)


})
 promise
 // .then(res => opener(res.link))k
 .then(res => console.log(res))
 .catch(rej => {
    
    const retry = (retryAttempts = 105) => { 

          if (retryAttempts < 0) {
              return Promise.reject(new Error("Exceeded maximum retries "));
          }
      console.log("Attempting, " + retryAttempts + " attempts left.");
      setTimeout( () => {search(color) ,  retry(retryAttempts - 1)} , 1000)
}
    retry()
})
...
// naje array , inseret into array wanted item , if array is 0 refresh , if  =1 clcick on link 
